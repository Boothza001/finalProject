/* MPU6050 Basic Example Code
 ...
 Hardware setup:
 MPU6050 Breakout --------- Arduino ******ESP32
 3.3V --------------------- 3.3V    
 SDA ----------------------- A4     ******GPIO21
 SCL ----------------------- A5     ******GPIO22
 GND ---------------------- GND
...
#include <Wire.h>
// #include <Adafruit_GFX.h>        ปิดไว้ก่อน
// #include <Adafruit_PCD8544.h>    ปิดไว้ก่อน
...
// pin 6 - LCD reset (RST)
// Adafruit_PCD8544 display = Adafruit_PCD8544(9, 8, 7, 5, 6);   ปิดไว้ก่อน
...
void setup()
{
  ...
  // display.begin();         // Initialize the display     ปิดไว้ก่อน
  // display.setContrast(58); // Set the contrast     ปิดไว้ก่อน
  // display.setRotation(2);  //  0 or 2) width = width, 1 or 3) width = height, swapped etc.      ปิดไว้ก่อน
...
{  
  ...
  if(readByte(MPU6050_ADDRESS, INT_STATUS) & 0x01) {  // check if data ready interrupt
    ...
    Serial.print("X :        "); Serial.print(gyrox, 1);Serial.print("                                                       ");Serial.print("Y :        "); Serial.println(gyroy, 1); //***************
